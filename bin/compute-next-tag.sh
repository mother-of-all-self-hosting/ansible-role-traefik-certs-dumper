...
#
# Tags look like `v<traefik-certs-dumper version>-<release>`:
#
# - if defaults/main.yml points at a traefik-certs-dumper version that has
#   never been released, the release counter restarts at 0 (`v2.11.4-0`)
# - otherwise the counter is incremented (`v2.11.4-1`), but only if something
...
# The literal version value that Renovate bumps, not one of the variables
# derived from it (`traefik_certs_dumper_container_image_tag` and friends hold
# Jinja expressions, which say nothing about the version on their own).
version="$(sed -nE 's|^traefik_certs_dumper_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the traefik-certs-dumper version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v2.11.4`), which must
# not be doubled in the tag. Stripping any `v` before prepending one keeps this
